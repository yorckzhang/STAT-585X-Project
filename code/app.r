...
educationTidy <- function(education){
  ...
  education_tidy$Year <- gsub("2011.2015",replacement = "2015",education_tidy$Year)
...
educationTrendsTidy <- function(education){
  ...
  #here we gather the data and split to give us a tidy format of our data 
  education_trends_tidy <- education_trends_tidy %>% gather(key="Type-Year",value="Number",4:23) %>% 
    separate(`Type-Year`,into=c("Type","Year"),sep="-")
  
  education_trends_tidy$Year <- gsub("2011.2015",replacement = "2015",education_trends_tidy$Year) 
...
populationTidy <- function(population){
  ...
  #selecting the important variables
  population_tidy <- population %>% select(1,2,3,16,76,81)%>%data.frame()
  
  #renaming some columns to common pattern
  names(population_tidy)[4:6] <- c("Population-2015","BirthRate-2015","DeathRate-2015")
  
  #here we gather the data and split to give us a tidy format of our data 
  population_tidy<- population_tidy %>% gather(key="Type-Year",value="Value",4:6) %>% separate(`Type-Year`,into=c("Type","Year"))
...
populationTrendsTidy <- function(population){
  ...
  #selecting the important variables
  population_trends_tidy <- population %>% select(1,2,3,11,12,13,14,15,16,72,73,74,75,76,77,78,79,80,81)%>%data.frame()
  ...
  #here we gather the data and split to give us a tidy format of our data 
  population_trends_tidy<- population_trends_tidy %>% gather(key="Type-Year",value="Number",4:19) %>% separate(`Type-Year`,into=c("Type","Year"))
...
povertyTidy <- function(poverty){
  ...
  poverty_tidy <- poverty_tidy %>% gather(key="Type",value="Value",4:6) %>% mutate(Year="2015")
...
unemploymentTidy <- function(unemployment){
  ...
  unemployment_tidy <- unemployment %>% select(1,2,3,43) %>% data.frame()
  #renaming some columns to common pattern
  unemployment_tidy <- unemployment_tidy %>% rename("FIPS"=`FIPStxt`,"Area_Name"=`Area_name`,"Unemployment Rate-2015"=`Unemployment_rate_2015`) %>% 
...
unemploymentTrendsTidy <- function(unemployment){
  ...
  unemployment_trends_tidy <- unemployment %>% select(1,2,3,11,15,19,23,27,31,35,39,43) %>% data.frame()
  
  #renaming some columns to common pattern
  unemployment_trends_tidy <- unemployment_trends_tidy %>% rename("FIPS"=`FIPStxt`,"Area_Name"=`Area_name`)
  ...
  #here we gather the data and split to give us a tidy format of our data 
  unemployment_trends_tidy <- unemployment_trends_tidy %>% gather(key="Type-year", value="Number",4:12) %>% 
...
education_trends_US <- education_trends_StatesOnly %>% group_by(Type,Year) %>% summarize(count = sum(Number)/length(unique(State)))
...
population_trends_US <- population_trends_StatesOnly %>% group_by(Type,Year) %>% summarize(count=sum(Number)/length(unique(State)))
poverty_trends_US <- poverty_trends_StatesOnly %>% group_by(Type) %>% summarize(count=sum(Value)/length(unique(State)))
unemployment_trends_US <- unemployment_trends_StatesOnly %>% group_by(Type,Year) %>% summarize(count=sum(Number)/length(unique(State)))
...
server <- function(input, output) {
  ...
  output$Trends <- renderPlotly({
    ...
    if(input$SocioeconomicIndicator=="Education"){
      ...
      plot <- dataSelected %>% ggplot(aes(x=Year,y=count))+geom_point(aes(colour=Type))+
        geom_path(aes(group=Type,colour=Type))+
